...
def fetch_xforce_timestamp(cve_no):
    api_key = "NTYzYmMwNTEtMTM5Ny00NjJhLWFmODEtYjA1Y2QzY2Y4NDFiOmZiNTE3Y2E4LTE4NWMtNDFhMC04NDViLTE4NDk3YTAxMDA5Nw=="
    xforce_url = "https://api.xforce.ibmcloud.com/vulnerabilities/search/"
    headers = {
        'Accept': 'application/json',
        'Authorization': 'Basic ' + api_key,
    }
    response = requests.get(xforce_url + cve_no, headers=headers)
    json_obj = response.json()[0]
    print(json_obj['reported'])
    return json_obj['reported']

if __name__ == '__main__':
    fetch_dockerfiles()
    fetch_xforce_timestamp("CVE-2017-5897")
